    ...
    def execute_sync(self, dry_run=False):
        ...
        for op in self.operations:
            ...
            try:
                if op[0] == 'copy':
                    if dry_run:
                        self.gui.log(f"[模拟] 复制 {os.path.basename(op[1])} → {op[2]}", "INFO")
                        continue
                    os.makedirs(os.path.dirname(op[2]), exist_ok=True)
                    shutil.copy2(op[1], op[2])
                    if self.gui:
                        self.gui.log(f"已复制 {os.path.basename(op[1])} → {op[2]}", "INFO")
                    
                elif op[0] == 'mkdir':
                    if dry_run:
                        self.gui.log(f"[模拟] 创建目录 {os.path.basename(op[1])}", "INFO")
                        continue
                    os.makedirs(op[1], exist_ok=True)
                    self.gui.log(f"已创建目录 {os.path.basename(op[1])}", "INFO")
                    
                elif op[0] == 'update':
                    if dry_run:
                        self.gui.log(f"[模拟] 更新 {os.path.basename(op[1])}", "INFO")
                        continue
                    shutil.copy2(op[1], op[2])
                    self.gui.log(f"已更新 {os.path.basename(op[1])}", "INFO")
                    
                elif op[0] == 'conflict':
                    if dry_run:
                        self.gui.log(f"[模拟] 冲突文件 {os.path.basename(op[2])}", "WARNING")
                        continue
                    shutil.copy2(op[1], op[2])
                    self.gui.log(f"已创建冲突副本 {os.path.basename(op[2])}", "WARNING")
                    
                # 更新进度
                if self.gui:
                    self.gui.update_progress(processed, total_ops)
                    
            except PermissionError as e:
                self.gui.log(f"权限不足: {os.path.basename(op[1])}", "ERROR")
                self._handle_permission_error(op)
            except FileNotFoundError as e:
                self.gui.log(f"文件不存在: {os.path.basename(op[1])}", "ERROR")
            except shutil.SameFileError:
                self.gui.log(f"文件未变化: {os.path.basename(op[1])}", "INFO")
        
        self.gui.log(f"同步完成，共处理 {total_ops} 项操作", "INFO")
...
class SyncGUI(tk.Tk):
    ...
    def _calculate_differences(self, src_tree, dst_tree):
        """计算差异数量"""
        diff_count = 0
